...
| WinSCP 集成 | 支持在设置中配置 WinSCP 路径，并从当前连接、连接列表、管理弹窗或右键菜单一键打开 SFTP |
| 启动优化 | 延后数据库初始化、懒加载 xterm/SSH/SFTP、拆分核心与延迟事件绑定，改善冷启动体验 |
...
| `save-winscp-path` | R→M | 保存 WinSCP 可执行文件路径 |
| `open-in-winscp` | R→M | 使用 WinSCP 打开指定连接的 SFTP |
...
### 配置方式
1. 打开 **设置 / 数据库设置** 弹窗
2. 在 **WinSCP 集成** 区域选择 `WinSCP.exe` 路径
3. 点击 **保存**

### 打开入口
- **设置页**：点击 **打开当前连接 SFTP**
- **左侧连接列表**：悬停连接项后点击 WinSCP 图标
- **管理连接弹窗**：点击对应连接的 `WinSCP` 按钮
- **连接右键菜单**：点击 **用 WinSCP 打开**

### 说明
- 支持密码认证连接直接传递用户名、端口和密码
- 私钥认证会在本地临时生成私钥文件供 WinSCP 使用，应用退出时自动清理
- 若未配置 WinSCP 路径或路径无效，界面会显示错误提示
...
- **2026-04-26**：
  - 新增 WinSCP 集成功能：可在设置页配置 WinSCP 路径并持久化保存
  - 新增多入口一键打开 SFTP：支持从设置页当前连接、左侧连接列表、管理连接弹窗、连接右键菜单使用 WinSCP 打开
  - 补充私钥认证兼容：私钥连接会生成临时密钥文件供 WinSCP 使用，并在应用退出时自动清理
  - 优化启动性能：主窗口不再阻塞等待数据库初始化，xterm、SSH、SFTP 相关逻辑改为按需加载
  - 优化渲染层初始化：拆分核心事件与延迟事件绑定，减少首屏初始化开销
